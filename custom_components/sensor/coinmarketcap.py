...
class CoinMarketCapSensor(Entity):
    ...
    def device_state_attributes(self):
        ...
        mbtc_price = round(float(self._ticker.get('price_btc'))*1000, 1)

        if mbtc_price < 10.00:
            mbtc_price = round(float(self._ticker.get('price_btc'))*1000, 2)
        if mbtc_price < 1.00:
            mbtc_price = round(float(self._ticker.get('price_btc'))*1000, 3)
        if mbtc_price < 0.100:
            mbtc_price = round(float(self._ticker.get('price_btc'))*1000, 4)
